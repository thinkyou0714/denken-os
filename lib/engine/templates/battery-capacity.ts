...
function buildFrom(current: number, hours: number): GenerationResult | null {
...
export const batteryCapacity: Template = {
  topic: "蓄電池の容量",
  subject: "機械",
  ...
  generate(rng) {
    return buildFrom(pick(I_SET, rng), pick(T_SET, rng));
  },
  generateFrom(params) {
    const { current, hours } = params;
    if (current === undefined || hours === undefined) return null;
    return buildFrom(current, hours);
  },
};
