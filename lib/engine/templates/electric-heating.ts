...
export const electricHeating: Template = {
  ...
  generate(rng) {
    return buildFrom(pick(M_SET, rng), pick(DT_SET, rng), pick(P_SET, rng), pick(ETA_SET, rng));
  },
  generateFrom(params) {
    const { mass, delta_theta, power, efficiency } = params;
    if (mass === undefined || delta_theta === undefined || power === undefined || efficiency === undefined) {
      return null;
    }
    return buildFrom(mass, delta_theta, power, efficiency);
  },
};
