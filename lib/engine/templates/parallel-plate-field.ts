...
export const parallelPlateField: Template = {
  topic: "平行平板コンデンサの電界",
  subject: "理論",
  ...
  generate(rng) {
    return buildFrom(pick(V_SET, rng), pick(D_SET, rng));
  },
  generateFrom(params) {
    const { voltage, gap } = params;
    if (voltage === undefined || gap === undefined) return null;
    return buildFrom(voltage, gap);
  },
};
