...
export const lightingDesign = defineTemplate<Params>({
  topic: "照明設計（光束法）",
  subject: "機械",
  ...
  paramOrder: ["illuminance", "area", "lumen", "utilization", "maintenance"],
  draw(rng) {
    return {
      illuminance: pick(E_SET, rng),
      area: pick(A_SET, rng),
      lumen: pick(F_SET, rng),
      utilization: pick(U_SET, rng),
      maintenance: pick(M_SET, rng),
    };
  },
  buildFrom({ illuminance: E, area: A, lumen: F, utilization: U, maintenance: M }) {
