...
type Params = {
  percent_resistance: number;
  percent_reactance: number;
  power_factor: number;
};

export const transformerVoltageRegulation = defineTemplate<Params>({
  topic: "変圧器の電圧変動率",
  subject: "機械制御",
  ...
  paramOrder: ["percent_resistance", "percent_reactance", "power_factor"],
  draw(rng) {
    const [cos] = pick(PF, rng);
    return {
      percent_resistance: pick(P_SET, rng),
      percent_reactance: pick(Q_SET, rng),
      power_factor: cos,
    };
  },
  buildFrom({ percent_resistance: p, percent_reactance: q, power_factor: cos }) {
