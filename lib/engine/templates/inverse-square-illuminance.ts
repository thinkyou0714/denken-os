...
export const inverseSquareIlluminance: Template = {
  topic: "点光源の照度(逆二乗則)",
  subject: "機械",
  ...
  generateFrom(params) {
    const { luminous_intensity, distance } = params;
    if (luminous_intensity === undefined || distance === undefined) return null;
    return buildFrom(luminous_intensity, distance);
  },
};
