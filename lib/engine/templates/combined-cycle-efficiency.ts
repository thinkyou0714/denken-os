...
export const combinedCycleEfficiency = defineTemplate<Params>({
  ...
  paramOrder: ["eta_gas", "eta_steam"],
  draw(rng) {
    return {
      eta_gas: pick(ETA_G_SET, rng),
      eta_steam: pick(ETA_S_SET, rng),
    };
  },
  buildFrom({ eta_gas: etaG, eta_steam: etaS }) {
