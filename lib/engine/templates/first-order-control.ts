...
export const firstOrderControl: Template = {
  topic: "一次遅れ系のステップ応答",
  subject: "機械制御",
  ...
  generate(rng) {
    return buildFrom(pick(K_SET, rng), pick(A_SET, rng), pick(T_SET, rng));
  },
  generateFrom(params) {
    const { gain, step_size, time_constant } = params;
    if (gain === undefined || step_size === undefined || time_constant === undefined) return null;
    return buildFrom(gain, step_size, time_constant);
  },
};
