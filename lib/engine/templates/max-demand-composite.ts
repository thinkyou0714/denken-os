...
type Params = {
  capacity_a: number;
  demand_factor_a: number;
  capacity_b: number;
  demand_factor_b: number;
  diversity: number;
};

export const maxDemandComposite = defineTemplate<Params>({
  topic: "合成最大需要電力",
  subject: "電力管理",
  ...
  paramOrder: ["capacity_a", "demand_factor_a", "capacity_b", "demand_factor_b", "diversity"],
  draw(rng) {
    return {
      capacity_a: pick(CAPA_SET, rng),
      demand_factor_a: pick(DF_SET, rng),
      capacity_b: pick(CAPA_SET, rng),
      demand_factor_b: pick(DF_SET, rng),
      diversity: pick(DIV_SET, rng),
    };
  },
  buildFrom({ capacity_a: capA, demand_factor_a: dfA, capacity_b: capB, demand_factor_b: dfB, diversity: div }) {
