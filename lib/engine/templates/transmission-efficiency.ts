...
function buildFrom(Pr: number, Ps: number): GenerationResult | null {
...
export const transmissionEfficiency: Template = {
  topic: "送電効率",
  subject: "電力",
  ...
  generate(rng) {
    ...
    return buildFrom(Pr, Ps);
  },
  generateFrom(params) {
    const { received_power, sent_power } = params;
    if (received_power === undefined || sent_power === undefined) return null;
    return buildFrom(received_power, sent_power);
  },
};
