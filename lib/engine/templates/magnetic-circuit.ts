...
export const magneticCircuit = defineTemplate<Params>({
  topic: "磁気回路の磁束",
  subject: "理論",
  ...
  paramOrder: ["turns", "current", "reluctance"],
  draw(rng) {
    return {
      turns: pick(N_SET, rng),
      current: pick(I_SET, rng),
      reluctance: pick(RM_SET, rng),
    };
  },
  buildFrom({ turns: n, current: i, reluctance: rm }) {
