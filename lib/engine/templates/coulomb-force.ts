...
export const coulombForce: Template = {
  ...
  generate(rng) {
    return buildFrom(pick(Q_SET, rng), pick(Q_SET, rng), pick(R_SET, rng));
  },
  generateFrom(params) {
    const { charge1, charge2, distance } = params;
    if (charge1 === undefined || charge2 === undefined || distance === undefined) return null;
    return buildFrom(charge1, charge2, distance);
  },
};
