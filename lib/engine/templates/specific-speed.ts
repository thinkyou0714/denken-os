...
export const specificSpeed: Template = {
  topic: "水車の比速度",
  subject: "電力",
  ...
  generate(rng) {
    ...
    return buildFrom(h, p, n);
  },
  generateFrom(params) {
    const { head, power, speed } = params;
    if (head === undefined || power === undefined || speed === undefined) return null;
    return buildFrom(head, power, speed);
  },
};
