...
/** 今後 days 日の復習予定件数（カードの due を日別に集計）。index0=今日。 */
export function reviewForecast(views: Iterable<FsrsView>, nowMs: number, days = 7): number[] {
  const out = new Array<number>(days).fill(0);
  const todayIdx = Math.floor(nowMs / DAY_MS);
  for (const v of views) {
    const diff = Math.floor(v.dueMs / DAY_MS) - todayIdx;
