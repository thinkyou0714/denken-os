...
import { type LearningOptions, renderQuestion } from "./learning-ui.js";
import { disclosure, emptyState } from "./study-ui.js";
import { button, h, input, link, notice, panel, select, table } from "./ui.js";

export async function renderLibrary(root: HTMLElement, launch?: (options: LearningOptions) => void) {
  const wrap = panel("学びたい内容から探す");
  const query = input("論点・出典・問題番号・年度を検索");
  query.input.type = "search";
  query.input.placeholder = "例：三相交流、変圧器、電力";
  ...
  const render = () => {
    ...
    list.append(
      h(
        "p",
        { class: "lab-meta", role: "status" },
        `${rows.length}問${rows.length > 100 ? "（先頭100問を表示）" : ""}`,
      ),
    );
    if (!rows.length)
      list.append(
        emptyState("条件に合う教材がありません", "キーワードを短くするか、科目を「すべて」に変えてみてください。"),
      );
    const results = h("ul", { class: "study-library-list" });
    for (const p of rows.slice(0, 100)) {
      const open = button(mode.input.value === "通常学習の教材" ? "この問題を解く" : "検証用に開く", () => {
        detail.replaceChildren();
        if (mode.input.value === "通常学習の教材") {
          if (launch) {
            launch({ ids: [p.id], autoStart: true });
            return;
          }
      ...
      results.append(
        h(
          ...
          h(
            "div",
            {},
            h("span", { class: "study-subject-tag" }, p.subject),
            h("h4", {}, p.topic),
            h(
              "p",
              { class: "lab-meta" },
              `${p.format === "descriptive" ? "記述" : p.format === "numeric" ? "計算" : "選択"} · ${p.id}`,
            ),
          ),
          open,
        ),
  ...
  wrap.append(
    h("div", { class: "lab-grid" }, query.field, subject.field),
    disclosure("教材の確認状態を切り替える", mode.field),
    list,
  );
  ...
  root.append(disclosure("収録範囲を確認する", coverage));
  ...
  root.append(disclosure("原典と補助教材のリンク", resources));
  const offline = panel(
    "オフライン教材",
  ...
  root.append(disclosure("オフライン教材を管理する", offline));
}
