...
import { cardToReviewState, FsrsReviewScheduler, FsrsScheduler, reviewStateToCard } from "../../lib/scheduler/fsrs.js";
...
const PLUS_30D = Date.UTC(2026, 0, 31);
...
describe("FsrsReviewScheduler（FSRS を Scheduler として永続化経路に乗せる）", () => {
  ...
  it("Learning 状態(誤答直後)を往復保持し、Card を直接続けたのと同じ次回になる（reps 復元の誤分類を防ぐ）", () => {
    // ts-fsrs では New から again は State.Learning（reps=1 でも Review ではない）。
    // 旧実装は reps>0 を一律 Review に復元し、Learning の短期分岐ではなく長期分岐を取って
    // 間隔が約10倍に膨らんでいた（誤答カードが翌日でなく約10日後に再出題される）。
    const cardSched = new FsrsScheduler(0.9);
    const c1 = cardSched.review(cardSched.init(new Date(NOW)), "again", new Date(NOW));

    // 学習状態がドメイン型へ往復保持される（reps からの近似ではない）。
    expect(cardToReviewState(c1).state).toBe(c1.state);

    // 真値: Card を直接続ける（シリアライズ無し）。往復: state→ReviewState→Card 復元して続ける。
    const direct = cardSched.review(c1, "good", new Date(PLUS_30D));
    const round = cardSched.review(reviewStateToCard(cardToReviewState(c1)), "good", new Date(PLUS_30D));
    expect(round.scheduled_days).toBe(direct.scheduled_days);
    expect(round.stability).toBeCloseTo(direct.stability, 6);
  });

  it("行ストア往復で FSRS の state 列が保持される（B3 + state）", () => {
    const sched = new FsrsReviewScheduler(0.9);
    const s1 = sched.review(sched.init(NOW), "again", NOW); // Learning
    const reloaded = rowToReviewState(reviewStateToRow("u1", "理論", s1));
    expect(reloaded.state).toBe(s1.state);
  });
});
