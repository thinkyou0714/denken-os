...
1. **ローカルとCIを同じコマンドに寄せる**
   ...
   - データ量・形式・監修状況の棚卸しは `npm run audit:status` で確認し、公開前判定では `npm run release:check`（= `verify` + strict audit）を使う。閾値調整は `npm run audit:status -- --min-validated=50 --min-descriptive=10` のように明示する。JSON不正やschema不一致はファイル名と理由を出し、CLIクラッシュではなく監査結果として扱う。
