...
電験 (電気主任技術者試験) 学習 OS。学習コンテンツ + Web アプリ (Next.js) + データパイプラインで
「電験合格までのアルゴリズム化」を目指す。**pre-alpha**。

- **Stack**: TypeScript / Next.js (`web/`), Node `>=20`, Vitest, Supabase, Biome (lint/format).
...
- **Test**: `npm test` (→ `vitest run`). **Build**: `npm run build` (in `web/`).
