...
- **出題範囲の徹底網羅（深掘り100・第2弾）**: `docs/strategy/ideas/15-syllabus-coverage-100.md`。
  ...
  - **web/problems.json 589→790問（+201）**: 法規79問・MC49問。88topic。
  - **問題IDを内容由来の安定ハッシュに刷新**（`G-{FNV1a(topic|係数)}`）: テンプレ追加・並び替えで
    IDがズレて既存ユーザーの間違いノートが別問題を指す事故を構造的に防止（Codexレビュー対応。
    旧連番IDからの移行で一度だけ既存参照が孤児化するが、アプリは未解決IDを安全に除外する）。
