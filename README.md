...
| 領域 | 実装 | 仕様 |
|---|---|---|
| 問題生成＆検証エンジン | `lib/engine/`（決定論ソルバ＋検算＋出典＋CLI、テンプレ**14種＝全6科目網羅**: 理論/電力/機械/法規/電力管理/機械制御。MC＋numeric＋descriptive形式。**係数に応じた動的難易度**と**誤答ごとの着眼を解説へ自動付与**） | `docs/automation/01` |
...
> `problem-schema.json`(ajv) と zod 定義の**ドリフトをテストで検知**。Supabase スキーマは RLS 付きで `supabase/migrations/` に DDL を用意（実投稿/永続化の実体は認証取得後にアダプタ接続）。行⇔ドメインのマッピングは純関数として往復テスト済み（`rubric` 含む全フィールドが保存往復で保たれる）。
> 二次=記述(descriptive)は自動採点せず**自己採点**（模範解答＋採点観点を提示）。`data/problems/` に手検算済みの validated 問題（T-0001〜0009、全6科目を網羅）を収録。
...
npm test                                                  # ユニットテスト（269件）
npm run test:coverage                                     # カバレッジ（lib コアに閾値ゲート: line/func/stmt 80%, branch 75%）
```
...
- **記述式の部分点ルーブリック採点**（二種二次）: 記述問題は①自分の解答を入力→②模範解答と並置→③採点観点ごとに◯△✕で自己採点→④得点率・必須観点の充足・弱点観点を判定（`lib/study/rubric.ts`）。合格ライン（60%）到達を弱点ループ・科目別到達度へ接続。さらに採点観点に**能力軸タグ（立式/計算/単位/論述/作図）**を持たせ、セッション横断で集計して「**立式は強いが論述が弱い**」を記述特有の弱点として可視化する。設計と残りの100アイデアは [`docs/automation/15-descriptive-secondary.md`](docs/automation/15-descriptive-secondary.md)。
...
| 問題エンジン＋検証＋オフライン学習アプリ MVP | 2026-05 | ✅ done（テンプレ14種・全6科目・記述ルーブリック・音声学習・合格逆算） |
| M3 弱点診断 prototype | 2026-08 | ✅ 先行実装（SM-2/FSRS＋科目別到達度＋観点別弱点） |
| M1 過去問 1 年分を Obsidian markdown 化 | 2026-06 | in progress（vault 書き出し基盤あり・コンテンツ拡充中） |
| M2 Next.js skeleton + Supabase schema | 2026-07 | partial（RLS 付き DDL を `supabase/migrations` に用意・UI 未接続） |
