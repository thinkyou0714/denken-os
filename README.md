...
- **記述式の部分点ルーブリック採点**（二種二次）: 記述問題は①自分の解答を入力→②模範解答と並置→③採点観点ごとに◯△✕で自己採点→④得点率・必須観点の充足・弱点観点を判定（`lib/study/rubric.ts`）。合格ライン（60%）到達を弱点ループ・科目別到達度へ接続。設計と残りの100アイデアは [`docs/automation/15-descriptive-secondary.md`](docs/automation/15-descriptive-secondary.md)。
