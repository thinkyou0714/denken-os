...
| **オフライン学習アプリ（学習OS）** | `web/`（PWA・タブ型: 学習/復習/模試/進捗/公式/設定。**図解(回路図/ベクトル図/ブロック図のインラインSVG)＋構造化解説**・**FSRS＋4段階評価**・弱点優先＋科目ドリル・**間違いノート**・**時間制限模試(合格判定)**・科目別到達度・試験カウントダウン・数式整形・自動生成220問。esbuild・Service Worker） | README ビジョン |
...
タブ型の学習OS（**学習 / 復習 / 模試 / 進捗 / 公式 / 設定**）:

- **学習**: 弱点優先 or 科目ドリル。**図解（回路図・ベクトル図・ブロック図・特性曲線のインラインSVG）**＋解答→即解説（着眼点/公式/代入/計算/ポイント・数式整形）→**FSRS の4段階自己評価**（again/hard/good/easy）。
- **復習**: FSRS 期限の復習キュー＋**間違いノート**（誤答した問題を再演習＝想起練習）。
- **模試**: **時間制限・合格ライン60%判定・科目別内訳**で本番を再現。
- **進捗**: 科目別到達度・弱点TOP5・7日復習見込み・**試験カウントダウン**・日次目標。
- **公式 / 設定**: 公式集、試験日・1日目標・FSRS目標保持率・データリセット。

記憶状態は FSRS で localStorage に保存。Service Worker により**オフラインでも動作**します（バックエンド不要）。
学習法と試験構造は [`docs/strategy/exam-structure.md`](docs/strategy/exam-structure.md)、学習エンジンの改善計画100は
[`docs/strategy/ideas/06-exam-mastery-100.md`](docs/strategy/ideas/06-exam-mastery-100.md) を参照。
