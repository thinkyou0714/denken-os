...
| 領域 | 実装 | 仕様 |
|---|---|---|
| 問題生成＆検証エンジン | `lib/engine/`（決定論ソルバ＋検算＋出典＋CLI、**テンプレ88種＝全6科目**: 一次 理論/電力/機械/法規＋二次 電力管理/機械制御。過去問頻出テーマを網羅。MC＋numeric＋descriptive形式＋図解SVG） | `docs/automation/01` |
...
| **オフライン学習アプリ（学習OS）** | `web/`（PWA・タブ型: 学習/復習/模試/質問/進捗/公式/設定。**図解(回路図/ベクトル図/ブロック図のインラインSVG)＋構造化解説**・**FSRS＋4段階評価**・弱点優先＋科目ドリル・**間違いノート**・**時間制限模試(合格判定)**・科目別到達度・試験カウントダウン・数式整形・自動生成790問。esbuild・Service Worker） | README ビジョン |
...
npm run build:problems                                    # 全88テンプレから web/problems.json を再生成（790問・内容由来の安定ID）
