...
| **オフライン学習アプリ（学習OS）** | `web/`（PWA・タブ型: 学習/復習/模試/質問/進捗/公式/設定。**図解(回路図/ベクトル図/ブロック図のインラインSVG)＋構造化解説**・**FSRS＋4段階評価**・弱点優先＋科目ドリル・**間違いノート**・**時間制限模試(合格判定)**・科目別到達度・試験カウントダウン・数式整形・自動生成996問。**試験日逆算スケジューリング（直前モード）・科目別合格予測・本番再現模試（科目別実時間・一次4科目/二次合算判定）・二次の問題選択式**。esbuild・Service Worker） | README ビジョン |
